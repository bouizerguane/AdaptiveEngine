...
export default function GraphEditor() {
    ...
    useEffect(() => {
        ...
        conceptList.forEach((concept, index) => {
            ...
            concept.exigences?.forEach(targetConcept => {
                if (validConceptIds.has(targetConcept.id)) {
                    initialEdges.push({
                        id: `e-${concept.id}-${targetConcept.id}`,
                        source: concept.id,
    ...
    }, [activeTree, moduleId, chapitreId]);
    ...
    const getActiveTitle = () => {
        ...
        if (!courseId) return 'Knowledge Graph Global';
        if (activeTree) {
            if (chapitreId) {
                const mod = activeTree.modules?.find(m => m.id === moduleId);
                const chap = mod?.chapitres?.find(c => c.id === chapitreId);
            ...
            <div className="flex-1 bg-white border border-slate-200 rounded-xl shadow-inner relative overflow-hidden min-h-[500px]">
                ...
                {nodes.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-center p-10">
                        ...
                        <p className="text-slate-400 text-sm mt-2">Veuillez sélectionner un cours ou un module contenant des concepts.</p>
                    </div>
                ) : (
