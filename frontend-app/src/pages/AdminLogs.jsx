    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-slate-800">Logs Systeme</h1>
                    <p className="text-slate-500 text-lg mt-2">Journaux d'activite et traces des microservices.</p>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3 lg:grid-cols-4">
                    <div className="relative lg:col-span-2">
                        ...
                        <input
                            ...
                            className="w-full rounded-xl border border-slate-300 bg-white py-2.5 pl-10 pr-3 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                        />
                    </div>
                    <select
                        value={level}
                        onChange={(event) => setLevel(event.target.value)}
                        className="rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                    >
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {logs.length > 0 ? logs.map((log, index) => (
                                <tr key={index} className="hover:bg-slate-50">
                                    ...
                                    <td className="p-4 font-semibold text-slate-700">{log.service}</td>
                                    <td className="p-4 text-slate-600">{log.message}</td>
                                </tr>
                            )) : (
                                <tr>
                                    <td colSpan="4" className="p-8 text-center text-slate-400">Aucun log trouve.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
