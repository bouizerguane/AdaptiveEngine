...
const reportJsonPath = path.join(__dirname, 'adaptive-v5-test-report.json');
const reportMdPath = path.join(__dirname, 'adaptive-v5-test-report.md');
...
async function runNoDataCase(session, course) {
  ...
  try {
    ...
    assertStrategy(test, adaptivePath, 'SUPPORTIVE');
    test.actual = { learnerProfile: compactProfile(profile), pedagogicalStrategy: compactStrategy(adaptivePath.pedagogicalStrategy) };
...
async function runNeedsRemediationCase(session, course, concepts) {
  ...
  try {
    ...
    assertNonEmpty(test, 'profileExplanation', profile?.profileExplanation);
    assertStrategy(test, adaptivePath, 'RECOVERY');
    test.actual = { learnerProfile: compactProfile(profile), pedagogicalStrategy: compactStrategy(adaptivePath.pedagogicalStrategy) };
    test.adaptivePathResponse = adaptivePath;
  } catch (error) {
    failCase(test, error);
...
async function runProgressingCase(session, course, concepts) {
  ...
  try {
    ...
    assertStrategy(test, adaptivePath, 'STANDARD');
    test.actual = { learnerProfile: compactProfile(profile), pedagogicalStrategy: compactStrategy(adaptivePath.pedagogicalStrategy) };
    test.adaptivePathResponse = adaptivePath;
  } catch (error) {
    failCase(test, error);
...
async function runHighPerformingCase(session, course, concepts) {
  ...
  try {
    ...
    assertTrue(test, 'masteryScore calculable', Number.isFinite(Number(profile?.masteryScore)));
    assertNonEmpty(test, 'profileExplanation', profile?.profileExplanation);
    assertStrategy(test, adaptivePath, 'ADVANCED');
    test.actual = {
      nextAction: adaptivePath.nextAction,
      learnerProfile: compactProfile(profile),
      pedagogicalStrategy: compactStrategy(adaptivePath.pedagogicalStrategy),
    };
    test.adaptivePathResponse = adaptivePath;
  } catch (error) {
    failCase(test, error);
...
function assertStrategy(test, adaptivePath, expectedType) {
  const strategy = adaptivePath?.pedagogicalStrategy;
  assertEquals(test, 'pedagogicalStrategy.strategyType', expectedType, strategy?.strategyType);
  assertNonEmpty(test, 'pedagogicalStrategy.strategyExplanation', strategy?.strategyExplanation);
  assertNonEmpty(test, 'pedagogicalStrategy.recommendedSequence', strategy?.recommendedSequence);
  assertNonEmpty(test, 'pedagogicalStrategy.tutoringMessageHint', strategy?.tutoringMessageHint);
}
...
function renderMarkdownReport(data) {
  ...
  for (const test of data.cases) {
    ...
    lines.push('```');
    lines.push('');
    lines.push('Pedagogical strategy:');
    lines.push('```json');
    lines.push(JSON.stringify(compactStrategy(test.adaptivePathResponse?.pedagogicalStrategy), null, 2));
    lines.push('```');
    lines.push('');
  }
  return lines.join('\n');
